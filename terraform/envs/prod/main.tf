...
# prod 환경 — 루트 모듈
#
# ⚠️ 학습용 리포입니다. 이 환경은 **apply하지 않습니다** (비용).
#    `make plan ENV=prod` 로 "같은 모듈이 다른 값으로 어떻게 그려지는지"만 봅니다.
#
# [dev와 무엇이 다른가]
#   main.tf  : 완전히 동일 — 모듈 호출 구조는 환경과 무관합니다
#   tfvars   : 노드 크기·개수, 로그 보관 기간, API 공개 범위가 다릅니다
#   versions.tf : 백엔드 key가 eks-lab/prod/... 로 분리됩니다
#
# 이 "main.tf는 같고 tfvars만 다르다"가 디렉터리 분리 방식의 핵심이자 약점입니다.
# 모듈 호출을 바꾸면 두 파일을 모두 고쳐야 합니다 (docs/day-12-modules.md 참고).
#
# [루트 모듈이 하는 일: 조립만]
